...
In this example, we will use two variables: - vacc - whether a person is vaccinated against COVID-19 (0 = no, 1 = yes) - mspi - average score of the Conspiracy Mentality Scale
...
In this example, we will use two variables: - dyst - this item measures to what extent individual would reject a homosexual relationship of a family member (1 = definitely would accept, 2 = would rather accept, 3 = would rather reject, 4 = definitely would reject) - disg_sens - average score of a scale measuring sensitivity to several disgusting stimuli (rotten vegetables, vomits et al.)
...
- party - political party preferences (ruling party = PiS, opposition = (KO, Lewica, PSL, PL2050), radicals = (Konfederacja, Kukiz), other = smaller parties, nonVote = non voting) - sys_jus - average score of System Justification Scale
...
The results in the table are extremely difficult to interpret. - First, they are on the log-odds scale. - Second, they represent change in odds relative to the reference category (the one that is not included in the table, here "nonVoting").

```{r}
fit_categorical
```

Plotting wil give us a better picture.
