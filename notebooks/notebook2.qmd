...
Because posterior is just a list of numbers it can be easily summarised with any function that you use for descriptive statistics in R. For example, to obtain posterior mean, we simply write:
...
You can also plot the results. *as.numeric* is workaround used here because there is no model object in the namespace. Later, when we will fit models with *brms* it will not be necessary.
